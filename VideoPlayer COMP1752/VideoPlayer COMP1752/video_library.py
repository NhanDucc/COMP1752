...
def remove_image(key):          # Function to remove the image of a library item
    try:                        # Try to remove the image of the library item
        item = library[key]     # Get the library item
        item.image_path = None  # Remove the image path of the library item
    except KeyError:            # If the key does not exist
        return                  # Return
